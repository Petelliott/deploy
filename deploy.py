    ...
    def get_config():
        ...
        if new_cnf_tme > conf_time:
            ...
            if "ssh_key" in config:
                config["git_env"]["GIT_SSH_COMMAND"] = \
...
def make_handler(get_config):
    ...
    class WebhookHandler(http.server.BaseHTTPRequestHandler):
        def do_POST(self):
            ...
            # the following line will be used in the future
            ...
            if os.path.isdir("./"+self.path):
                subprocess.run(
                    ["git", "-C", "./"+self.path,
                     "checkout", config["projects"][self.path]["branch"]]
                )
                subprocess.run(
                    ["git", "-C", "./"+self.path, "pull",
                     "git@github.com:{}.git".format(self.path)],
                    env=config["git_env"]
                )
            else:
                subprocess.run(
                    ["git", "clone", "git@github.com:{}.git".format(self.path),
                     "./"+self.path], env=config["git_env"]
                )
                subprocess.run(
                    ["git", "-C", "./"+self.path, "checkout",
                     config["projects"][self.path]["branch"]]
                )
                subprocess.run(
                    ["git", "-C", "./"+self.path, "pull"],
                    env=config["git_env"]
            ...
            if self.path in instances:
                ...
                subprocess.run(
                    ["pkill", "-TERM", "-P",
                     str(instances[self.path].pid)]
                )
                instances[self.path].wait()

            instances[self.path] = subprocess.Popen(
                config["projects"][self.path]["script"],
                cwd="./"+self.path, shell=True
            )

            print("deployed", self.path)
